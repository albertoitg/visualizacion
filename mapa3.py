...
# Municipal Particular Subvencionado Particular Pagado Corporacion

tipo = "Corporacion"

data = pd.read_csv("ingreso.csv")
data = pd.read_excel("ingresos_por_tipo_colegio.xlsx", tipo)
...
#quantile_bounds = np.percentile(data['SUELDO'], [0, 20, 40, 60, 80, 100])

# 4. Definir colormap y normalizador uniforme y por cuantiles
colormap = plt.cm.OrRd_r

norm = plt.Normalize(vmin=270000, vmax=7589134.7)
data['color_hex'] = data['ingreso'].apply(lambda x: mcolors.to_hex(colormap(norm(x))))

# norm = BoundaryNorm(quantile_bounds, ncolors=colormap.N)
# data['color_hex_quantiles'] = data['SUELDO'].apply(
#     lambda x: mcolors.to_hex(colormap(norm(x)))
# )

# # Clasificación Fisher-Jenks
# fisher_jenks = FisherJenks(data['SUELDO'], k=7)
# norm = BoundaryNorm(fisher_jenks.bins, ncolors=colormap.N)

# # Asignar color
# data['color_hex_fisher'] = data['SUELDO'].apply(
#     lambda x: mcolors.to_hex(colormap(norm(x)))
# )
...
tree.write(f"mapa_heatmap_ingreso_{tipo}_{modelo[modelo_seleccionado - 1]}.svg")
